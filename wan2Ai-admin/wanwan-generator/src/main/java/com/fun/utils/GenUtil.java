...
import com.fun.domain.GenConfig;
import com.fun.domain.ColumnInfo;
...
import java.util.*;
import static com.fun.utils.FileUtil.SYS_TEM_DIR;
...
public class GenUtil {
    ...
    public static String download(List<ColumnInfo> columns, GenConfig genConfig) throws IOException {
        ...
        // String tempPath =SYS_TEM_DIR + "wan2Ai-admin-gen-temp" + File.separator + genConfig.getTableName() + File.separator;
        String tempPath = SYS_TEM_DIR + "wan2Ai-admin-gen-temp" + File.separator + genConfig.getTableName() + File.separator;
        Map<String, Object> genMap = getGenMap(columns, genConfig);
        TemplateEngine engine = TemplateUtil.createEngine(new TemplateConfig("template", TemplateConfig.ResourceMode.CLASSPATH));
        // 生成后端代码
        ...
        for (String templateName : templates) {
            Template template = engine.getTemplate("admin/" + templateName + ".ftl");
            String filePath = getAdminFilePath(templateName, genConfig, genMap.get("className").toString(), tempPath + "wan2Ai-admin" + File.separator);
        ...
        for (String templateName : templates) {
            Template template = engine.getTemplate("front/" + templateName + ".ftl");
            String path = tempPath + "wanwan-web" + File.separator;
